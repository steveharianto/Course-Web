...
import { getCourse } from "../firebaseUtils";
...
export default function Page() {
    ...
    const fetchCourse = async () => {
        const tempCourse = await getCourse();
                    ...
                    <div className=" min-h-screen w-full flex flex-col">
                        ...
                        <div className="flex justify-between p-10">
                            ...
                            <button
                                ...
                                className="border-[1px] border-black h-fit py-2 px-8 rounded bg-[#f88f8f]"
                            >
                                Sign Out
                            </button>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="#17594a" className="bi bi-person-circle mx-auto h-[10em]" viewBox="0 0 16 16">
                            <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0" />
                            <path fillRule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1" />
                        </svg>
                        <p className="mx-auto mt-4 font-bold text-xl">{user.username}</p>
                        <p className="mx-auto">{user.role}</p>
                        <p className="mx-auto text-xl">{user.email}</p>
                        <p className="text-xl font-bold ms-10 mt-8">My Courses</p>
                        <div className="w-[95%] mx-auto flex justify-between flex-wrap">
                            {/* Course Card */}
                            {user.courses?.map((course, index) => {
                                const c = courseList.find((cr) => cr.id === course);
                                let progress = 0;
                                let total = c.chapters.length;

                                if (c.progress.hasOwnProperty(user.username)) {
                                    progress = c.progress[user.username].length;
                                }

                                if (c) {
                                    return (
                                        <div
                                            className="border-[1px] border-black shadow-lg w-[48%]  rounded-lg my-4 flex hover:cursor-pointer"
                                            key={index}
                                            onClick={() => {
                                                Cookies.set("currentClass", JSON.stringify(c));
                                        ...
                                        >
                                            ...
                                            <div className="w-[60%] flex flex-col p-4">
                                                ...
                                                <p className="text-xs my-1">
                                                    {progress}/{total} Chapter Completed
                                                </p>
                                                <div className="bg-[#f8f4f4] h-4 w-full">
                                                    <div className={`h-full w-[${Math.round((progress / total) * 100)}%] bg-[#17594a]`}></div>
                                                </div>
                                            </div>
